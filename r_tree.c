...
rtree_node **packSortedSlicesIntoNodes(slice *slices, int noOfSlices)
{
    ...
    for (int i = 0; i < ceil((float)NUM_POINTS / MAX_ENTRIES); i++) // Getting no of nodes
    {
    ...
    */
    ...
    x = 0;
    // Packing rectangles of all slices into nodes except the last two slices.
    // These nodes are packed to full extent
    ...
    */
    ...
    for (int i = 0; i < num_of_new_nodes; i++)
    {
        ...
        for (int j = 0; j < MAX_ENTRIES; j++)
        {
            ...
            add_rectangle_to_node(new_nodes[i], rects[rects_added]); // mbr is getting added
